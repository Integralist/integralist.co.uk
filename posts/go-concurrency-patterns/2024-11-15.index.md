...
> 💡 **TIP**\
> The [errgroup](https://pkg.go.dev/golang.org/x/sync/errgroup) package is like
> a `sync.WaitGroup` but with added capabilities for error handling and context
> cancellation; while a WaitGroup simply waits for a collection of goroutines to
> finish, an errgroup also collects any errors returned by those goroutines and
> can cancel all of them if one fails.
