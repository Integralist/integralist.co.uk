...
func main() {
	...
	for i := range 10 {
		g.Go(func() error {
			// Pass the errgroup context down to the worker.
			return worker(ctx, i)
		})
	}
	
	// Wait for all goroutines to finish.
	// g.Wait will return the first non-nil error returned by a goroutine.
	if err := g.Wait(); err != nil {
		fmt.Printf("\nAn error occurred: %v\n", err)
	} else {
		fmt.Println("\nAll workers done successfully.")
	}
}

// worker accepts a context to enable cancellation.
func worker(ctx context.Context, id int) error {
	fmt.Printf("Worker %d starting\n", id)
	var workDuration time.Duration

	// Assign different work durations to simulate a realistic scenario
	switch {
	case id < 3:
		// These workers are fast and should complete successfully
		workDuration = time.Duration(id+1) * 100 * time.Millisecond
	case id == 4:
		// This worker takes a bit longer and will fail
		workDuration = 500 * time.Millisecond
	default:
		// These workers are slow and should be cancelled
		workDuration = 2 * time.Second
	}

	select {
	case <-time.After(workDuration):
		// The simulated "work" has finished.
		// Now we check if this specific worker is supposed to produce an error.
		if id == 4 {
			fmt.Printf("Worker %d encountering an error\n", id)
			return errors.New("something went wrong in worker 4")
		}
		fmt.Printf("Worker %d done\n", id)
...
https://play.golang.com/p/Xhdl5XU5Iwq
